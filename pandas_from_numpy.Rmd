...
```{python}
mmr_series = pd.Series(mmrs, index=country_codes, name='mmr')
mmr_series
```
...
Notice too that the Series has a name, that we set with the `name=` argument to the `pd.Series` constructor:

```{python}
mmr_series.name
```

Think then of the Series as an object that asssociates an array of values (`.values`) and corresponding labels for each value (`.index`), and a name (`.name`).
...
```{python}
hcepp_series = pd.Series(hcepps, index=country_codes, name='hcepp')
hcepp_series
```
...
We can get the `mmr` series by name, by indexing directly into the data frame, like this:
